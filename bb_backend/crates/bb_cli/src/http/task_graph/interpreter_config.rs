...
/// Optional overrides for InterpreterOptions, used by daemon CLI to pass
/// through user-specified flags.
#[derive(Debug, Clone, Default)]
pub(crate) struct InterpreterOverrides {
...
/// 构建 InterpreterOptions，消除 tg_create_run 和 tg_resume_gate 中的重复代码。
/// When `overrides` is `None`, uses all defaults (backward-compatible).
pub(crate) fn build_interpreter_opts(
    root: &std::path::Path,
    project: String,
    run_id: String,
    overrides: Option<&InterpreterOverrides>,
) -> task_graph::InterpreterOptions {
    let agent = overrides
        .and_then(|o| o.agent.as_deref())
        .unwrap_or("bb-pm");
    let mcp_servers = overrides
        .map(|o| o.mcp_servers.as_slice())
        .unwrap_or(&[]);

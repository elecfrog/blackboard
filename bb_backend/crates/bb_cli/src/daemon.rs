...
fn parse_graph_ref(graph: &str) -> Result<(&str, &str)> {
    ...
    if parts.len() != 2 || parts[0].is_empty() || parts[1].is_empty() {
        anyhow::bail!(
            "invalid --graph format: '{}'. Expected '<scope>/<id>', e.g. 'system/inbox-batch-cleanup' or 'project/my-graph'",
            graph
        );
    }
    let scope = parts[0];
    if scope != "system" && scope != "project" {
        anyhow::bail!(
            "invalid graph scope: '{}'. Must be 'system' or 'project'",
            scope
        );
    }
    Ok((scope, parts[1]))
...
fn load_graph(
    ...
    match scope {
        ...
        "project" => task_graph::read_project_graph(workspace.root(), project, id)
            .with_context(|| format!("load project graph '{}' for project '{}'", id, project)),
...
fn run_graph(
    ...
    let overrides = InterpreterOverrides {
        ...
        codebuddy_path: Some(options.codebuddy.clone()),
        opencode_path: Some(options.opencode.clone()),
        agent: Some(options.agent.clone()),
        run_timeout_secs: Some(options.run_timeout_seconds),
        ..Default::default()
    };

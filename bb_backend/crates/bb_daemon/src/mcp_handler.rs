...
impl BbMcpHandler {
    pub const fn new(workspace: Arc<RwLock<Workspace>>) -> Self {
        Self { workspace }
    }
}
...
impl ServerHandler for BbMcpHandler {
    ...
    {
        let workspace = self.workspace.read().map_or_else(
            |_| {
...
fn convert_tools_list() -> Vec<Tool> {
    ...
    tools_array
        ...
        .filter_map(|tool_value| {
            ...
            tool.name = name.into();
            tool.description = description.map(std::convert::Into::into);

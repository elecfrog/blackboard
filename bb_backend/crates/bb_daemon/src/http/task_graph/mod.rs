...
use dto::{
    ...
    TgCatalogPatchBody, TgCatalogResponse, TgCreateRunBody, TgForkBody, TgGraphRef,
    TgGraphResponse, TgResumeGateBody, TgRunCheckpointsResponse, TgRunDetailResponse,
    TgRunEventsResponse, TgRunResponse, TgRunStatusResponse, TgRunsListResponse,
...
pub use schedules::{dispatch_due_schedules, spawn_schedule_dispatcher};
...
pub(super) async fn tg_list_catalog(
    ...
    let catalog = task_graph::list_graph_catalog(&root, &project)?;
    Ok(Json(TgCatalogResponse {
        graphs: catalog.graphs,
        groups: catalog.groups,
    }))
}

/// PATCH /api/projects/{project}/task-graphs/catalog — update catalog metadata
#[allow(dead_code)]
pub(super) fn tg_patch_catalog(
    State(state): State<AppState>,
    Path(project): Path<String>,
    Json(body): Json<TgCatalogPatchBody>,
) -> Result<Json<TgCatalogResponse>, TaskGraphApiError> {
    let root = current_workspace_root(&state)?;
    let catalog = task_graph::save_graph_catalog_index(&root, &project, body.into())?;
    Ok(Json(TgCatalogResponse {
        graphs: catalog.graphs,
        groups: catalog.groups,
    }))
...
pub(super) async fn tg_create_run(
    ...
    if run.created && !body.dry_run && run.run.status != task_graph::RunStatus::Queued {
        spawn_task_graph_run(root, project, run.run.id.clone());
    }

    Ok((run.status, Json(TgRunResponse { run: run.run })))
}

pub struct CreatedTaskGraphRun {
    pub(crate) run: task_graph::TaskGraphRunSummary,
    pub(crate) created: bool,
    pub(crate) status: StatusCode,
}

pub fn create_task_graph_run(
    root: &FsPath,
    project: &str,
...
const fn occupies_concurrency_slot(status: task_graph::RunStatus) -> bool {
    matches!(
        status,
...
pub fn dispatch_queued_task_graph_runs(
    root: &FsPath,
    project: &str,
    ...
    loop {
        ...
        for queued in queued_runs {
            ...
            if queued_run_expired(&queued, now) {
                handle_queued_run_timeout(root, project, &queued, &policy, now)?;
                progressed = true;
                continue;
            }
...
fn handle_queued_run_timeout(
    root: &FsPath,
    project: &str,
    run: &task_graph::TaskGraphRunSummary,
    policy: &task_graph::TaskGraphRunPolicy,
    now: DateTime<Utc>,
) -> Result<(), TaskGraphApiError> {
    if policy.queue_timeout_retry_enabled {
        let retry_count = queued_timeout_retry_count(root, project, &run.id)?;
        if retry_count < policy.max_queue_timeout_retries {
            retry_queued_run_timeout(root, project, run, policy, retry_count + 1, now)?;
            return Ok(());
        }
    }

    expire_queued_run(root, project, run, policy, now)
}

fn queued_timeout_retry_count(
    root: &FsPath,
    project: &str,
    run_id: &str,
) -> Result<u32, TaskGraphApiError> {
    let count = task_graph::list_run_events(root, project, run_id)?
        .into_iter()
        .filter(|event| event.kind == "run_queue_timeout_retry")
        .count();
    Ok(count as u32)
}

fn retry_queued_run_timeout(
    root: &FsPath,
    project: &str,
    run: &task_graph::TaskGraphRunSummary,
    policy: &task_graph::TaskGraphRunPolicy,
    ...
    let next_deadline_at = queue_deadline_at(policy);
    match task_graph::update_queued_run_deadline(root, project, &run.id, next_deadline_at.clone()) {
        Ok(updated) => {
            task_graph::append_run_event(
                root,
                project,
                &run.id,
                0,
                "run_queue_timeout_retry",
                None,
                "Queued run exceeded max queue wait and was retried",
                json!({
                    "graph_scope": run.graph_ref.scope,
                    "graph_id": run.graph_ref.id,
                    "queued_at": run.queued_at,
                    "previous_queue_deadline_at": run.queue_deadline_at,
                    "queue_deadline_at": next_deadline_at,
                    "timed_out_at": now.to_rfc3339(),
                    "retry_count": retry_count,
                    "max_queue_timeout_retries": policy.max_queue_timeout_retries,

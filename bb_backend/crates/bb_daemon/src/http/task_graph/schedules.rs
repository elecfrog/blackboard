...
pub struct TgSchedulesResponse {
    pub(crate) schedules: Vec<TaskSchedule>,
}

#[derive(Debug, Serialize)]
pub struct TgScheduleResponse {
    pub(crate) schedule: TaskSchedule,
}

pub async fn tg_list_schedules(
...
pub async fn tg_create_schedule(
    State(state): State<AppState>,
    Path(project): Path<String>,
...
pub async fn tg_patch_schedule(
    State(state): State<AppState>,
...
pub async fn tg_delete_schedule(
    State(state): State<AppState>,
    Path((project, schedule_id)): Path<(String, String)>,
...
pub async fn tg_run_schedule_now(
    State(state): State<AppState>,
    Path((project, schedule_id)): Path<(String, String)>,
...
pub fn spawn_schedule_dispatcher(workspace: Workspace) {
...
pub fn dispatch_due_schedules(workspace: &Workspace) -> Result<usize, TaskGraphApiError> {

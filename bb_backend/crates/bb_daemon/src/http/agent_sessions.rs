...
use bb_core::task_graph::{build_pi_mcp_adapter_config_content, resolve_scripts_dir};
use chrono::Utc;
...
pub struct AgentSessionEventsQuery {
    since: Option<u64>,
}
...
#[derive(Debug, Serialize)]
pub struct AgentSessionEventsResponse {
    events: Vec<AgentEvent>,
}

#[derive(Debug)]
pub struct AgentSessionApiError(AgentSessionError);
...
pub async fn create_agent_session(
    ...
    if !matches!(runtime.as_str(), "opencode" | "codex" | "codebuddy" | "pi") {
        return Err(AgentSessionApiError(AgentSessionError::InvalidInput(
            format!("direct project chat does not support runtime {runtime}"),
        )));
    }
    ...
    let pi_mcp_config_content =
        build_pi_mcp_adapter_config_content(&root, &agent, &overrides.mcp_servers);
    let request = AgentTurnRequest {
        ...
        startup_timeout: agent_session::effective_agent_startup_timeout(Duration::from_secs(
            body.timeout_secs.unwrap_or(DIRECT_CHAT_TIMEOUT_SECS),
        )),
    };
...
pub async fn read_agent_session_events(
    State(state): State<AppState>,
    Path((project, session_id)): Path<(String, String)>,
...
pub async fn stream_agent_session_events(
    State(state): State<AppState>,
    Path((project, session_id)): Path<(String, String)>,
    Query(query): Query<AgentSessionEventsQuery>,

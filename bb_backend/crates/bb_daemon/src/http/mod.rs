...
pub fn app_with_options(workspace: Workspace, options: HttpServeOptions) -> Router {
    ...
    Router::new()
        ...
        .route(
            "/api/projects/{project}/runtime-models/{runtime}",
            get(list_runtime_models_handler),
        )
...
pub fn inspect_workspace_folder_root(
    state: &AppState,
    root: &FsPath,
...
async fn list_runtime_models_handler(
    State(state): State<AppState>,
    Path((project, runtime)): Path<(String, String)>,
) -> Result<Json<RuntimeModelsResponse>, ApiError> {
    state.workspace()?.open_project(&project)?;
    let runtime = runtime.trim().to_ascii_lowercase();
    match runtime.as_str() {
        "opencode" => {
            let result = tokio::task::spawn_blocking(list_opencode_models_blocking)
                .await
                .map_err(|err| {
                    ApiError(InboxError::InvalidInput(format!(
                        "runtime model lookup failed: {err}"
                    )))
                })?;
        ...
        "pi" => {
            let result = tokio::task::spawn_blocking(list_pi_models_blocking)
                .await
                .map_err(|err| {
                    ApiError(InboxError::InvalidInput(format!(
                        "runtime model lookup failed: {err}"
            ...
            Ok(Json(RuntimeModelsResponse {
                runtime,
                source: "pi --list-models".to_string(),
                models,
                error,
            }))
        }
        "codex" | "codebuddy" => Ok(Json(RuntimeModelsResponse {
            runtime,
            source: "manual".to_string(),
            models: Vec::new(),
            error: None,
        })),
        _ => Err(ApiError(InboxError::InvalidInput(format!(
            "unsupported runtime `{runtime}`"
        )))),
    }
}

fn list_opencode_models_blocking() -> Result<Vec<String>, String> {
    let program = bb_core::platform::resolve_spawn_program("opencode");
    let output = Command::new(&program)
        .arg("models")
        .output()
        .map_err(|err| format!("failed to run `opencode models`: {err}"))?;
...
fn list_pi_models_blocking() -> Result<Vec<String>, String> {
    let program = bb_core::platform::resolve_spawn_program("pi");
    let output = Command::new(&program)
        .arg("--list-models")
        .env("PI_OFFLINE", "1")
        .output()
        .map_err(|err| format!("failed to run `pi --list-models`: {err}"))?;
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).trim().to_string();
        return Err(if stderr.is_empty() {
            format!("`pi --list-models` exited with {}", output.status)
        } else {
            stderr
        });
    }

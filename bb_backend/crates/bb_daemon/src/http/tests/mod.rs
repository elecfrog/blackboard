...
fn write_inbox_json_note(path: impl AsRef<std::path::Path>, title: &str, topic: &str) {
    let document = json!({
        "schema_version": 1,
        "title": title,
...
async fn agent_tools_list_returns_static_catalog() {
    ...
    assert!(
        tools
            .iter()
            .any(|tool| tool["id"] == "pi"
                && tool["npm_package"] == "@earendil-works/pi-coding-agent")
    );
}
...
async fn rest_lists_projects_and_routes_inbox_by_project() {
    ...
    write_inbox_json_note(
        workspace
            .projects_root()
            .join("demo/inbox/2026-05-04-codex-test.json"),
        "Seed JSON note",
        "seed",
    );
    fs::write(
        workspace
            .projects_root()
            .join("demo/inbox/2026-05-04-codex-legacy.md"),
        "ignored",
    )
    .unwrap();
    let app = app(workspace);
    ...
    assert_eq!(
        ...
        "2026-05-04-codex-test.json"
    );

    let response = app
        .clone()
        .oneshot(
            Request::get("/api/projects/demo/inbox/notes/2026-05-04-codex-test.json")
                .body(Body::empty())
                .unwrap(),
        )
        .await
        .unwrap();
    assert_eq!(response.status(), StatusCode::OK);
    let note = json_response(response).await;
    assert_eq!(note["document"]["title"], "Seed JSON note");
    assert!(note["content"]
        .as_str()
        .unwrap()
        .contains("created a JSON note"));
    ...
    assert!(created["name"]
        .as_str()
        .unwrap()
        .ends_with("opencode-rest-create.json"));
    assert_eq!(
...
async fn rest_routes_project_requests_to_external_data_root() {
  ...
  "lanes": [
    ...
    write_inbox_json_note(
        external.join("inbox/2026-05-08-note.json"),
        "External",
        "external",
    );
...
async fn rest_rejects_traversal_with_json_error() {
    ...
    let response = app(workspace)
        .oneshot(
            Request::get("/api/projects/demo/inbox/notes/..%2Ftickets%2Factive%2Fx.json")
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
async fn tg_schedule_crud_routes() {
    ...
    assert!(created["state"]["next_run_at"].is_string());
...
#[tokio::test]
async fn tg_queued_dispatcher_retries_expired_deadline_before_failing() {
    let (_temp, workspace) = tg_fixture();
    let app = app(workspace.clone());
    let mut graph = project_graph_with_run_policy_json(false, 1, true, 60_000);
    graph["metadata"]["run_policy"]["queue_timeout_retry_enabled"] = json!(true);
    graph["metadata"]["run_policy"]["max_queue_timeout_retries"] = json!(1);
    create_project_graph(app.clone(), json!({ "graph": graph })).await;
    ...
    let first_expired_at = (Utc::now() - ChronoDuration::minutes(1)).to_rfc3339();
    let mut run_json: Value =
        serde_json::from_str(&fs::read_to_string(&run_path).unwrap()).unwrap();
    run_json["queue_deadline_at"] = json!(first_expired_at);
    fs::write(&run_path, serde_json::to_string_pretty(&run_json).unwrap()).unwrap();
    ...
    let run = bb_core::task_graph::read_run(workspace.root(), "demo", &queued_id).unwrap();
    assert_eq!(run.status, bb_core::task_graph::RunStatus::Queued);
    assert_ne!(
        run.queue_deadline_at.as_deref(),
        Some(first_expired_at.as_str())
    );
    let events =
        bb_core::task_graph::list_run_events(workspace.root(), "demo", &queued_id).unwrap();
    assert_eq!(

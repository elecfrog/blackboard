...
impl CodexAppServer {
    ...
    pub fn spawn(codex_path: &str, cwd: &Path, model: Option<&str>) -> Result<Self> {
        let (codex_program, codex_prefix_args) =
            bb_core::platform::resolve_codex_spawn_command(codex_path);
    ...
    fn request(&mut self, method: &str, params: Value) -> Result<Value> {
        ...
        loop {
            ...
            if let Some(resp_id) = msg.get("id").and_then(serde_json::Value::as_i64) {
                if resp_id == id {
                    if let Some(err) = msg.get("error") {
                        ...
                        let message = err
                            ...
                            .and_then(|v| v.as_str())
                            .unwrap_or("unknown");
                        anyhow::bail!("codex {method} failed: {message} (code={code})");
                    }
    ...
    pub fn shutdown(mut self) {
        ...
        let _ = bb_core::platform::terminate_child_process_tree(&mut self.child);

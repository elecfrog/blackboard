...
use super::super::eval::render_prompt_template;
use super::super::interpreter::{InterpreterOptions, RunOutcome};
use super::super::outcome::NodeOutcome;
...
use super::navigation::resolve_next_nodes;

// ─── Sub-Graph Node ─────────────────────────────────────────────────────────────

pub(super) fn execute_sub_graph_node(
    ...
    // 注意：这里使用 interpreter::execute_run 进行递归调用
    // 子 run 是一个独立的 Coordinator 实例
    let child_opts = InterpreterOptions {
    ...
    let child_outcome = super::super::interpreter::execute_run(&child_opts)?;

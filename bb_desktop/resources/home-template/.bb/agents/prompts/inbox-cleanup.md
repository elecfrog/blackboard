...
Inbox note name: {note_name}

任务：
1. 阅读这条 JSON inbox handoff，并在同一 project 下寻找最匹配的已有 ticket。`content` 是展示投影；如果工具返回 `document`，以 `document` 为结构化事实。
...
4. 如果无法明确归属，不要创建 ticket，不要删除 note；如有结构化 inbox 更新工具才可标注待人工归属或信息不足，否则保留并报告。
...
7. 遵守 bb-pm agent 文件里的所有规则，优先使用 bb-server MCP/CLI 能力，避免手改 ticket JSON、inbox JSON 或写 `extra.attachments`。
